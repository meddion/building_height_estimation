...
class Score:
    ...
    def get_score(self, *, print_detailed_score=True):
        if self.precision_correct_area + self.precision_correct_area == 0:
            ...
            precision = (
                self.precision_correct_area
                / (self.precision_correct_area + self.precision_incorrect_area)
                * 100
            )
        recall = (
            self.recall_correct_area
            / (self.recall_correct_area + self.recall_incorrect_area)
            * 100
        )
    ...
    with open(file_path) as json_file:
        ...
        for s in shapes:
            ...
            if "group_id" not in s:
                raise Exception(
                    f"invalid file format (no 'group_id' field): {file_path}"
                )
            height = s["group_id"]
    ...
    for i in range(len(buildings)):
        for j in range(i + 1, len(buildings)):
            if check_bounding_box_intersection(buildings[i], buildings[j]):
                intersection = (
                    buildings[i].polygon.intersection(buildings[j].polygon).area
                )
                if (
                    intersection
                    / min(buildings[i].polygon.area, buildings[j].polygon.area)
                    > BUILDINGS_IOU_THRESHOLD
                ):
                    raise Exception(
                        f"two polygons in file {file_name} have iou larger than the threshold"
                    )
...
def get_score_for_one_file_buildings(gt_buildings, out_buildings):
    ...
    for out_building in out_buildings:
        ...
        if (
            intersection_area
            > out_building.polygon.area * BUILDING_CORRECT_AREA_THRESHOLD
        ):
    ...
    for gt_building in gt_buildings:
        ...
        if (
            intersection_area
            > gt_building.polygon.area * BUILDING_CORRECT_AREA_THRESHOLD
        ):
    ...
    for i, out_building in enumerate(out_buildings):
        for j, gt_building in enumerate(gt_buildings):
            if check_bounding_box_intersection(out_building, gt_building):
                intersection_area = out_building.polygon.intersection(
                    gt_building.polygon
                ).area
                if (
                    intersection_area
                    >= min(out_building.polygon.area, gt_building.polygon.area)
                    * BUILDING_CORRECT_AREA_THRESHOLD
                ):
