...
def debug_show_image(img, title="image"):
...
def shadow_detection(
    ...
    with rasterio.open(image_file) as f:
        metadata = f.profile
        # debug_show_image(f.read(1), "Original Image")
        img = rescale_intensity(
            np.transpose(f.read(tuple(np.arange(metadata["count"]) + 1)), [1, 2, 0]),
            out_range="uint8",
        )
        img = img[:, :, 0:3]
        debug_show_image(img, "Img after transform")
    ...
    debug_show_image(log_sr_img, "log_sr_img")
    ...
    debug_show_image(blurred_sr_img, "blurred_sr_img")
    ...
    debug_show_image(flattened_sr_img, "flattened_sr_img")
    ...
    # debug_show_image(shadow_mask, "shadow_mask")
    ...
    #     # debug_show_image(sahdow_mask_transp)
    #     dst.write(shadow_mask)

    shadow_mask_t = np.transpose(shadow_mask, [1, 2, 0])
    data = im.fromarray(shadow_mask_t)
    data.save(shadow_mask_file)
    return shadow_mask
...
def shadow_correction(image_file, shadow_mask_file, corrected_image_file, exponent=1):
    ...
    for i in range(img.shape[2]):
        ...
        shadow_stats = np.float32(
            np.mean(((shadow_area_mask**exponent) / np.sum(shadow_mask)))
            ** (1 / exponent)
        )
        non_shadow_stats = np.float32(
            np.mean(((non_shadow_area_mask**exponent) / np.sum(non_shadow_mask)))
            ** (1 / exponent)
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        raise ValueError("Provide input and output file paths")

    shadow_detection(sys.argv[1], sys.argv[2])
